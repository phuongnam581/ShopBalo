...
class CheckoutModel extends DBConnect{
    ...
    function saveOrder($created_at, $id_customer,$address){

        $sql = "INSERT INTO `order_cus`(`id`, `id_employ`, `created_at`, `id_customer`, `address`) VALUES ('',null,$created_at,$id_customer,'$address')";
        $result = $this->executeQuery($sql);
        return $result ? $this->getLastId() : false;
    }

    function saveOrderDetail($idOrder,$idProduct, $qty, $price){
        $sql = "INSERT INTO order_detail (product_code,value,quanlity_out,id_order)
